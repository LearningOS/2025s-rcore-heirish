...
impl TaskManager {
    ...
    fn get_task_syscall_number(&self, syscall_id: usize) -> usize {
        if syscall_id <= MAX_SYSCALL_ID {
            let inner = self.inner.exclusive_access();
            let current = inner.current_task;
            inner.tasks[current].task_syscalls[syscall_id]
...
pub fn get_task_syscall_number(syscall_id: usize) -> usize {
    TASK_MANAGER.get_task_syscall_number(syscall_id)
}
